...
* 对于没有被`ACCESS_ONCE()`保护的内存引用,不能假设编译器编译出来的指令与代码顺序一致.

* 不能假设独立的loads和stores会被以代码顺序发射到内存系统,这意味着对于:
...
>1写(store)内存屏障.
>
>  写屏障保证,系统中的其余部件看到的写屏障之前的写操作必定先于屏障之后的写操作完成.
>
>  写屏障只是影响写操作的次序,并不要求对读操作产生任何影响.
> 
...
>
>  数据依赖屏障只影响有互相关联的读操作,并不要求它对写操作,独立的读操作或重叠读操作产生影响.
>
